pipeline {
    ...
    stages {
        ...
        sh 'tar -xzvf /var/lib/jenkins/workspace/weatherapp/apache-maven-3.9.7-bin.tar.gz'
            }
        }
        stage('Compile Sample App') {
            steps {
            dir('/var/lib/jenkins/workspace/weatherapp/weatherapp/weatherapp_main'){
            sh '/var/lib/jenkins/workspace/weatherapp/apache-maven-3.9.7/bin/mvn compile'}
            }
        }
       stage('Test Sample App') {
            steps {
            dir('/var/lib/jenkins/workspace/weatherapp/weatherapp/weatherapp_main'){
            sh '/var/lib/jenkins/workspace/weatherapp/apache-maven-3.9.7/bin/mvn test'}
            }
        }
       stage('Package Sample App') {
            steps {
            dir('/var/lib/jenkins/workspace/weatherapp/weatherapp/weatherapp_main'){
            sh '/var/lib/jenkins/workspace/weatherapp/apache-maven-3.9.7/bin/mvn package'}
            }
        }
      stage('Upload Artifacts To Nexus') {
            steps {
            dir('/var/lib/jenkins/workspace/weatherapp/weatherapp/weatherapp_main/target'){
        ...
        stage('Deploy App To Server') {
            steps {
        sh 'sudo cp /var/lib/jenkins/workspace/weatherapp/weatherapp/weatherapp_main/target/weather-app.war apache-tomcat-9.0.89/webapps/'
        ...
        sh 'sudo "/var/lib/jenkins/workspace/weatherapp/apache-tomcat-9.0.89/bin/startup.sh"'
            }
        }
    }
}
